 ...
 * which walks with an explicit work stack (never recursion proportional to
 * graph size) and only ever over values that already passed the closed JSON
 * value grammar, so no cycle and no accessor can occur here. Prototypes
 * normalize to ordinary objects and arrays, every own string key — including
 * `"__proto__"` — is defined through `Object.defineProperty` so plain
 * assignment can never reach the inherited setter, and every container is
