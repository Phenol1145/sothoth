...
// bootstrap: the exact accepted revision-4 Source Facts — the eleven accepted
// Artifact Design registrations with `SOTHOTH-GRAPH-DOSSIER` and
// `SOTHOTH-DOCUMENT-INDEX-DOSSIER` at design revision 3 and the other nine at
// design revision 2, Architecture Baseline `SOTHOTH-ARCHITECTURE-BASELINE-0.1`
// revision 4, and the formal `SOTHOTH-RELEASE-SCOPE-BOM-0.1@4` — are fed to
...
// frozen revision-4 fixtures, including `sourceFactsDigest`, with the frozen
// `scripts/check-pre-design.mjs` as the independent replay reference.
//
// No revision-1, revision-2, or revision-3 feed and no historical fixture participates in
// this replay; the identity pins below fail the test if the live facts drift
// away from the accepted revision-4 state. This replay is implementation
...
// The exact revision-4 source-facts digests of the frozen fixtures.
const REVISION_4_CLOSURE_DIGEST =
  "sha256:fb1eaada77044d8f343afca2c96701a698e86d129cc61033e1647759e5513961";
const REVISION_4_SCOPE_DIGEST =
  "sha256:a5e8dab756cf987e91741e010ec7be653df39513650405680d961ec00659aa26";
...
async function loadRevision4Facts(phase: "closure" | "scope"): Promise<Facts> {
...
// document-index package over the exact revision-4 Dossier bytes, with index
...
function buildIndex(facts: Facts, documentKind: string): DocumentIndexProjectionV1 {
  ...
  if (!result.ok) {
    throw new Error(`revision-4 index build failed: ${canonicalJson(result.issues)}`);
  }
...
describe("revision-4 feed identity pins (no historical fixture feeds this replay)", () => {
  test("the live facts are exactly the accepted revision-4 Source Facts", async () => {
    const facts = await loadRevision4Facts("scope");
    expect(facts.registry.registryRevision).toBe(4);
    expect(facts.registrations.collectionRevision).toBe(4);
    ...
    expect(byComponent.get("@project-sothoth/graph")?.designRevision).toBe(3);
    expect(byComponent.get("@project-sothoth/document-index")?.designRevision).toBe(3);
    const others = [...byComponent.keys()].filter(
      (componentId) => componentId !== "@project-sothoth/graph" && componentId !== "@project-sothoth/document-index",
    );
    expect(others).toHaveLength(9);
    for (const componentId of others) {
      expect(byComponent.get(componentId)?.designRevision, componentId).toBe(2);
    }
    ...
    expect(scopeBom.bomRevision).toBe(4);
  });
});

describe("governance self-host replay over the exact revision-4 Source Facts", () => {
  test("the Design Closure projection is byte-identical to the frozen fixture and the frozen checker", async () => {
    const facts = await loadRevision4Facts("closure");
    const result = compileDesignClosureV1(governanceClosureInput(facts));
    expect(result.outcome).toBe("valid");
    expect(result.diagnostics).toEqual([]);
    expect(result.projection).not.toBeNull();
    expect(result.projection?.sourceFactsDigest).toBe(REVISION_4_CLOSURE_DIGEST);
  ...
  test("the Scope BOM Admissibility projection is byte-identical to the frozen fixture and the frozen checker", async () => {
    ...
    expect(result.diagnostics).toEqual([]);
    expect(result.projection).not.toBeNull();
    expect(result.projection?.sourceFactsDigest).toBe(REVISION_4_SCOPE_DIGEST);
  ...
  test("equality covers the full projection bytes and conclusions, not selected fields", async () => {
    const closureFacts = await loadRevision4Facts("closure");
    ...
    const scopeFacts = await loadRevision4Facts("scope");
