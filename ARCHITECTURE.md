...
|---|---|
| `@project-sothoth/contracts` | — (none; zero-dependency floor) |
| `@project-sothoth/core` | `@project-sothoth/contracts` |
| `@project-sothoth/graph` | `@project-sothoth/contracts`, `@project-sothoth/core` |
| `@project-sothoth/document-index` | `@project-sothoth/contracts`, `@project-sothoth/core`, `@project-sothoth/graph`, `mdast-util-from-markdown@2.0.2` (external, exact pin) |
| `@project-sothoth/selectors` | `@project-sothoth/contracts`, `@project-sothoth/core`, `@project-sothoth/document-index` |
| `@project-sothoth/governance` | `@project-sothoth/contracts`, `@project-sothoth/core`, `@project-sothoth/document-index`, `@project-sothoth/graph`, `@project-sothoth/selectors` |
| `@project-sothoth/planning` | `@project-sothoth/contracts`, `@project-sothoth/core`, `@project-sothoth/graph`, `@project-sothoth/selectors` |
| `@project-sothoth/profile-sdk` | `@project-sothoth/contracts`, `@project-sothoth/core` |
| `@project-sothoth/git` | `@project-sothoth/contracts`, `@project-sothoth/core` |
| `@project-sothoth/sdk` | `@project-sothoth/contracts`, `@project-sothoth/core`, `@project-sothoth/document-index`, `@project-sothoth/git`, `@project-sothoth/governance`, `@project-sothoth/planning`, `@project-sothoth/profile-sdk`, `@project-sothoth/selectors` |
| `@project-sothoth/cli` | `@project-sothoth/sdk` |

Notes verified from the manifests:

- `@project-sothoth/planning` does **not** depend on `@project-sothoth/document-index`; its inputs arrive as values.
...
- `@project-sothoth/contracts` declares zero runtime dependencies (the dependency floor).
...
1. **Foundation** — [`@project-sothoth/contracts`](docs/packages/contracts.md) owns the closed vocabulary; [`@project-sothoth/core`](docs/packages/core.md) owns canonical bytes, digests, and outcome aggregation. Nothing in the workspace may bypass them.
2. **Algorithms** — [`@project-sothoth/graph`](docs/packages/graph.md) provides meaning-free multigraph algorithms ordered by caller keys.
3. **Documents** — [`@project-sothoth/document-index`](docs/packages/document-index.md) projects CommonMark structure deterministically.
4. **Selection / compilation** — [`@project-sothoth/selectors`](docs/packages/selectors.md) (closed selector algebra), [`@project-sothoth/governance`](docs/packages/governance.md) (closure, admissibility, change plans, gate macros), [`@project-sothoth/planning`](docs/packages/planning.md) (dependency-wave schedules).
5. **Boundaries** — [`@project-sothoth/profile-sdk`](docs/packages/profile-sdk.md) (consumer-neutral profiles) and [`@project-sothoth/git`](docs/packages/git.md) (read-only source adapter) each sit on an explicit edge of the trust boundary.
6. **Facade** — [`@project-sothoth/sdk`](docs/packages/sdk.md) is the sole aggregate public library surface; it delegates every semantic operation and owns no domain truth.
7. **Operator adapter** — [`@project-sothoth/cli`](docs/packages/cli.md) composes the facade into eight explicit commands and owns process I/O and the frozen exit mapping.

## Boundary obligations

- **Purity:** `@project-sothoth/core` and `@project-sothoth/graph` import no filesystem, Git, process, network, consumer paths, FRACTA names, or external executables. `@project-sothoth/core`'s only sanctioned non-package import is `node:crypto`.
...
- **Adapter confinement:** all Git execution lives in `@project-sothoth/git` behind a frozen read-only subcommand allowlist; all process I/O and exit-code choice live in `@project-sothoth/cli`; the SDK and every library package never exit the process.
